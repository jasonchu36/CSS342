...
bool Account::getInsertCheck() const { return Check; }
...
bool Account::withdraw(int Amount, int FundsID) {
  if (FundTypes[FundsID].isWithdrawalEnough(Amount)) {
    ...
    if (FundsID == 0) {
      ...
      if (FundTypes[FundsID].getBalance() + FundTypes[0].getBalance() >=
          Amount) {
        int Leftover = Amount - FundTypes[FundsID].getBalance();
        FundTypes[FundsID].withdraw(Amount - Leftover);
      ...
      if (FundTypes[FundsID].getBalance() +
              FundTypes[FundsID + 1].getBalance() >=
          Amount) {
        int Leftover = Amount - FundTypes[FundsID].getBalance();
        FundTypes[FundsID].withdraw(Amount - Leftover);
